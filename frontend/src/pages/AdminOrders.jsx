...
const AdminOrders = () => {
    ...
    const fetchOrders = async () => {
        try {
            setLoading(true);
            const { data } = await axios.get('http://192.168.179.237:5002/api/orders');
            setOrders(data);
            setLoading(false);
        } catch (error) {
            console.error(error);
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        try {
            await axios.put(`http://192.168.179.237:5002/api/orders/${id}/status`, { status: newStatus });
            fetchOrders(); // Refresh
        } catch (error) {
    ...
    const handleSendToDelivery = async (order) => {
        ...
        try {
            const { data } = await axios.post(`http://192.168.179.237:5002/api/orders/${order._id}/delivery`);

...
const CheckoutForm = ({ product, variant, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validatePhone(formData.phone)) {
            setErrorMessage('Please enter a valid phone number (05, 06, or 07 followed by 8 digits)');
            return;
        }

        if (formData.deliveryType === 'home' && !formData.address) {
            setErrorMessage('Address is required for Home Delivery');
            return;
        }
    ...
    if (submitStatus === 'success') {
        return (
            <div className="bg-white p-8 rounded-3xl text-center space-y-4">
                ...
                <p className="text-gray-500">Thank you, {formData.name}. We will contact you at {formData.phone} shortly.</p>
            </div>
        );
    }
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 md:p-8 rounded-b-3xl md:rounded-3xl space-y-5">
            <h2 className="text-xl font-bold text-gray-900 mb-4">Express Checkout</h2>
            ...
            >
                {submitStatus === 'submitting' ? (
                    ...
                    <span>Confirm Order - {totalPrice} DA</span>
                )}
            </button>
        </form>
    );
